...
int main(int argc, char **argv) {
    ...
    makerAruco.DoMake();
    mappublish.run(10, 0);
    cerr << "MarkerAruce: Make Ok" << endl << endl;
    ...
    mappublish.run(100, 0);

    // Calibrate by SolverOptMk
    SolverOptMk solverOptMk(&datasetAruco);
    solverOptMk.DoCalib();
    cerr << "solverOptMk: result = " << datasetAruco.GetCamOffset() << endl << endl;
    mappublish.run(100, 0);

    Config::SetExtrinsics(datasetAruco.GetCamOffset().rvec, datasetAruco.GetCamOffset().tvec, datasetAruco.GetCamOffset().T());
    Config::SavePara();
