...
bool Dataset::ParseOdoData(const string _str, Se2& _odo, int& _id) {
    ...
    _odo.x = atof(vec_str[3].c_str()) * 1000.0;
    _odo.y = atof(vec_str[4].c_str()) * 1000.0;
...
void Dataset::CreateFrames() {
    ...
    std::cout << "load odometry OK " << std::endl;
    // build frame vector
    ...
    std::cout << "build frame vector, FrameCount = " << FrameCount << std::endl;
    return;
}
...
void DatasetAruco::CreateKeyFrames() {
    ...
    std::cout << "selected Frame: " << setpFrameSelected.size() << std::endl;
