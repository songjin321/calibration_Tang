...
void SolverInitMk::DoCalib() {
    ...
    Mat nvec_cg;
    cerr << "begin computer ground plane" << nvec_cg << endl;
    ...
    // z value from the measurement in 3D car model
    T_bc.at<float>(2,3) = 1570.0;
    cout << "T_bc " << T_bc << endl;
...
void SolverInitMk::ComputeGrndPlane(Mat &nvec_cg) {
    ...
    std::cerr << "measure Marker size = " << setMsrMk.size() << std::endl;
